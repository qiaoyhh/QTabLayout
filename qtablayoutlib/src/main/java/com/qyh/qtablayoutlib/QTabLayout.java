...
public class QTabLayout extends HorizontalScrollView {
    ...
    /**
     * Gravity used to fill the {@link QTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link QTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    private static boolean mChangeTabColor;// 是否使用tab渐变色
    ...
    public QTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public QTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.QTabLayout,
                defStyleAttr, R.style.QTabLayout_Default_Style);

        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.QTabLayout_q_tabIndicatorHeight, 0));
        // default value was 28dp
        mTabStrip.setSelectedIndicatorWidth(
                a.getDimensionPixelSize(R.styleable.QTabLayout_q_tabIndicatorWidth, dpToPx(28)));
        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.QTabLayout_q_tabIndicatorColor, 0));

        mTabStrip.setIndicatorAnimationSupport(a.getBoolean(R.styleable.QTabLayout_q_tabIndicatorAnimation, false));
        ...
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.QTabLayout_q_tabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.QTabLayout_q_tabPaddingEnd,
                mTabPaddingEnd);
        ...
        if (a.hasValue(R.styleable.QTabLayout_q_tabTextColor)) {
            // If we have an explicit text color set, use it instead
            mTabTextColors = a.getColorStateList(R.styleable.QTabLayout_q_tabTextColor);
        }

        if (a.hasValue(R.styleable.QTabLayout_q_tabSelectedTextColor)) {
        ...
        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.QTabLayout_q_tabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.QTabLayout_q_tabMaxWidth,
                INVALID_WIDTH);
        mTabBackgroundResId = a.getResourceId(R.styleable.QTabLayout_q_tabBackground, 0);
        mContentInsetStart = a.getDimensionPixelSize(R.styleable.QTabLayout_q_tabContentStart, 0);
        mMode = a.getInt(R.styleable.QTabLayout_q_tabMode, MODE_FIXED);
        mTabGravity = a.getInt(R.styleable.QTabLayout_q_tabGravity, GRAVITY_FILL);
        mIndicatorMarginTop = a.getDimensionPixelSize(R.styleable.QTabLayout_q_indicatorMarginTop, mIndicatorMarginTop);
        a.recycle();
    ...
    public void addTabView(@NonNull Tab tab, int position, boolean setSelected) {
        ...
        colorChangeView.setLayoutParams(layoutParams);
        tab.setCustomView(colorChangeView);
        ...
        setTabWidth(position, colorChangeView);
    }


    private void setTabWidth(int position, ColorChangeView colorChangeView) {
        ...
        int h = MeasureSpec.makeMeasureSpec(0,
                MeasureSpec.UNSPECIFIED);
        //手动测量一下
        colorChangeView.measure(w, h);
        params.width = colorChangeView.getMeasuredWidth() + tabView.getPaddingLeft() + tabView.getPaddingRight();
        //设置tabView的宽度
    ...
    public ColorChangeView getColorTrackView(int position) {
        return (ColorChangeView) getTabAt(position).getCustomView();
    }

    private void addTabFromItemView(@NonNull QTabItem item) {
    ...
    /**
     * Add a {@link QTabLayout.OnTabSelectedListener} that will be invoked when tab selection
     * changes.
     * <p>
    ...
    /**
     * Remove the given {@link QTabLayout.OnTabSelectedListener} that was previously added via
    ...
    /**
     * Remove all previously added {@link QTabLayout.OnTabSelectedListener}s.
     */
    ...
    /**
     * Returns the current mode used by this {@link QTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
    ...
    /**
     * The one-stop shop for setting up this {@link QTabLayout} with a {@link ViewPager}.
     * <p>
    ...
    private void addViewInternal(final View child) {
        if (child instanceof QTabItem) {
            addTabFromItemView((QTabItem) child);
        } else {
     ...
     * necessary calls back to the provided {@link QTabLayout} so that the tab position is
     * kept in sync.
     * <p>
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<QTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(QTabLayout tabLayout) {
        ...
        public void onPageScrollStateChanged(final int state) {
            ...
            mScrollState = state;
            QTabLayout tabLayout = mTabLayoutRef.get();
    ...
    public void tabScrolled(int position, float positionOffset) {
        ...
        ColorChangeView currentTrackView = getColorTrackView(position);
        ColorChangeView nextTrackView = getColorTrackView(position + 1);
    ...
    /**
     * A {@link QTabLayout.OnTabSelectedListener} class which contains the necessary calls back
     * to the provided {@link ViewPager} so that the tab position is kept in sync.
     */
    public static class ViewPagerOnTabSelectedListener implements QTabLayout.OnTabSelectedListener {
